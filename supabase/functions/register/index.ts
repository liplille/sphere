...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (linkErr || !linkData?.properties?.action_link) {
      ...
      return new Response(JSON.stringify({ error: "Erreur génération lien" }), {
        status: 500,
        headers,
      });
    }
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; color: #ffffff; background-color: #0a0a0a; padding: 40px; text-align: center;">
        ...
        <br>
        <a href="${actionLink}" style="display: inline-block; padding: 16px 32px; background-color: #00f3ff; color: #000; text-decoration: none; font-weight: bold; border-radius: 4px; font-size: 16px; margin-top: 10px; text-transform: uppercase; letter-spacing: 1px;">Confirmer mon alliance</a>
      </div>
    `;
